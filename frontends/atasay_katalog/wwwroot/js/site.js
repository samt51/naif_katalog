...
let cart = JSON.parse(sessionStorage.getItem('atasay_catalog_cart')) || [];
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById('customerOrderForm').addEventListener('submit', async function (event) {
        ...
        let pending;
        try { pending = JSON.parse(sessionStorage.getItem('atasay_pending_order')); } catch (_) { }
        if (!pending || pending.fingerprint !== fingerprint) {
            pending = { fingerprint, requestId: crypto.randomUUID() };
            sessionStorage.setItem('atasay_pending_order', JSON.stringify(pending));
        }
        ...
        const pdfText = pdfIsEnglish ? {
            ...
            quantity: 'Quantity', note: 'Note', moreStoneDetails: 'more stone details', footer: 'ATASAY | Customer product selection',
            other: 'OTHER'
        } : {
            ...
            quantity: 'Adet', note: 'Not', moreStoneDetails: 'taş detayı daha', footer: 'ATASAY | Müşteri ürün seçkisi',
            other: 'DİĞER'
        };
        ...
        async function getPdfDefinitionTranslation(entityType, entityId, fallback) {
            if (entityType === 'Category' && String(fallback).trim().toUpperCase() === 'SET') return 'SET';
            if (!pdfIsEnglish || !entityId) return translatePdfDefinition(fallback);
            ...
            try {
                const url = `/CustomerData/Translation?entityType=${encodeURIComponent(entityType)}&entityId=${encodeURIComponent(entityId)}&languageCode=en`;
        ...
        // Pre-load logo
        let logoImg = await loadImage('/images/brand/logo.svg');
        ...
        try {
            ...
            cart = [];
            updateCartUI();
            sessionStorage.removeItem('atasay_pending_order');
...
function updateCartUI() {
    consolidateCart();
    sessionStorage.setItem('atasay_catalog_cart', JSON.stringify(cart));
    ...
    for(let cat in grouped) {
        const isSet = cat.trim().toUpperCase() === 'SET';
        container.innerHTML += `<h6 class="mt-4 mb-2 fw-bold text-secondary border-bottom pb-1 ${isSet ? 'notranslate' : ''}" ${isSet ? 'translate="no"' : ''}>${escapeCatalogHtml(isSet ? 'SET' : cat)}</h6>`;
