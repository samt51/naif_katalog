...
namespace atasay_katalog.Core.Features.ProductFeature.Queries
{
    public class GetAllProductsQueryHandler : BaseHandler, IRequestHandler<GetAllProductsQueryRequest, ResponseDto<List<Product>>>
    {
        ...
        private string? ResolveExistingImageRelativePath(string? imageName)
        {
            ...
            var path = imageName.Replace('\\', '/').TrimStart('/');
            if (Uri.TryCreate(imageName, UriKind.Absolute, out var absolute))
            {
                var configured = _configuration["CatalogImageBaseUrl"];
                if (!Uri.TryCreate(configured, UriKind.Absolute, out var origin) ||
                    !origin.IsBaseOf(absolute)) return null;
                path = Uri.UnescapeDataString(origin.MakeRelativeUri(absolute).ToString());
            }
            if (path.StartsWith("images/katalog/", StringComparison.OrdinalIgnoreCase)) path = path[15..];
            if (path.Contains(':') || path.Contains('?') || path.Contains('#') || path.Split('/').Any(x => x is "" or "." or "..")) return null;
            return AllowedImageExtensions.Contains(Path.GetExtension(path).ToLowerInvariant()) ? path : null;
        }

        private string BuildImageUrlFromRelative(string localAddress, string relativePath)
            => "/CatalogMedia/Image?path=" + Uri.EscapeDataString(relativePath);
