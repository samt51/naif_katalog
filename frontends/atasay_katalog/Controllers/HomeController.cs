...
public class HomeController : Controller
{
    ...
    [Microsoft.AspNetCore.Authorization.Authorize]
    public async Task<IActionResult> Welcome([FromServices] atasay_katalog.Services.Concrete.HomeContentStore store)
    {
        var productsTask = _mediator.Send(new GetAllProductsQueryRequest
        {
            Page = 1, PageSize = 12, ColumnIndex = 0, OrderBy = "desc"
        });
        var categoriesTask = _mediator.Send(new atasay_katalog.Core.Features.CategoryFeature.Queries.GetAllCategoriesQueryRequest());
        ...
        return View(new HomePageViewModel
        {
            ...
            Categories = (categories?.data ?? []).Where(c => c.ParentId == 0).OrderBy(c => c.OrderIndex)
                .Select(c => new HomeCategoryCard(c.Id, c.Name, store.CategoryImage(c.Id, c.Name))).ToList(),
    ...
    public async Task<IActionResult> Detail(int id)
    {
        ...
        if (product != null)
        {
            ...
            try {
                ...
                var colorsResp = await _mediator.Send(new atasay_katalog.Core.Features.DefinitionFeature.Queries.GetAllMetalTypesQueryRequest());
                ViewBag.MetalTypes = colorsResp?.data;

                var puritiesResp = await _mediator.Send(new atasay_katalog.Core.Features.DefinitionFeature.Queries.GetAllMetalPuritysQueryRequest());
                ViewBag.Karats = puritiesResp?.data;

                var claritiesResp = await _mediator.Send(new atasay_katalog.Core.Features.DefinitionFeature.Queries.GetAllStoneClaritysQueryRequest());
                ViewBag.Clarities = claritiesResp?.data;

                var stonesResp = await _mediator.Send(new atasay_katalog.Core.Features.ProductFeature.Queries.GetAllStonesQueryRequest());
                ViewBag.Stones = stonesResp?.data;

                var stoneTypesResp = await _mediator.Send(new atasay_katalog.Core.Features.DefinitionFeature.Queries.GetAllStoneTypesQueryRequest());
    ...
    public async Task<IActionResult> LogCatalogAction([FromBody] CatalogActionLogRequest request)
    {
        ...
        {
            if (!_memoryCache.TryGetValue(CustomerCacheKey("Products"), out ResponseDto<List<Product>>? productsResponse))
        ...
        var response = await _mediator.Send(new atasay_katalog.Core.Features.UserActionLogFeature.Commands.Create.CreateUserActionLogCommandRequest
        {
            UserId = userId,
    ...
    public async Task<IActionResult> ConfirmOrder([FromBody] ConfirmOrderRequest request, [FromServices] IOrderStore orders)
    {
        ...
        if (saved.AccountId != account.Id) return Forbid();
    ...
    {
        if (_memoryCache.TryGetValue(CustomerCacheKey($"Product_{id}"), out Product? cachedProduct) && cachedProduct != null)
        {
        ...
        for (var page = 1; page <= 100; page++)
        {
            ...
            {
                _memoryCache.Set(CustomerCacheKey($"Product_{product.Id}"), product, TimeSpan.FromMinutes(10));
            }
    ...
    public async Task<IActionResult> Index(int? categoryId = null, string? search = null, decimal? minPrice = null, decimal? maxPrice = null, string? sortOrder = null, int page = 1)
    {
        if (HttpContext != null)
        {
            ...
            var categoriesListPaged = categoriesResponsePaged.isSuccess ? categoriesResponsePaged.data : new List<atasay_katalog.Models.CategoryDto>();
            ...
            try {
                var colorsTask = _mediator.Send(new atasay_katalog.Core.Features.DefinitionFeature.Queries.GetAllMetalTypesQueryRequest());
                var claritiesTask = _mediator.Send(new atasay_katalog.Core.Features.DefinitionFeature.Queries.GetAllStoneClaritysQueryRequest());
                var stoneTypesTask = _mediator.Send(new atasay_katalog.Core.Features.DefinitionFeature.Queries.GetAllStoneTypesQueryRequest());
                var stonesTask = _mediator.Send(new atasay_katalog.Core.Features.ProductFeature.Queries.GetAllStonesQueryRequest());
                await Task.WhenAll(colorsTask, claritiesTask, stoneTypesTask, stonesTask);

                var colorsResp = await colorsTask;
                ViewBag.MetalTypes = (colorsResp != null && colorsResp.isSuccess) ? colorsResp.data : new List<atasay_katalog.Core.Features.DefinitionFeature.Queries.MetalTypeDto>();

                var claritiesResp = await claritiesTask;
                ViewBag.Clarities = (claritiesResp != null && claritiesResp.isSuccess) ? claritiesResp.data : new List<atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneClarityDto>();

                var stoneTypesResp = await stoneTypesTask;
                ViewBag.StoneTypes = (stoneTypesResp != null && stoneTypesResp.isSuccess) ? stoneTypesResp.data : new List<atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneTypeDto>();

                var stoneResponse = await stonesTask;
                ViewBag.Stones = (stoneResponse != null && stoneResponse.isSuccess) ? stoneResponse.data : new List<atasay_katalog.Models.StoneDto>();
            } catch {
                ViewBag.MetalTypes = new List<atasay_katalog.Core.Features.DefinitionFeature.Queries.MetalTypeDto>();
                ViewBag.Clarities = new List<atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneClarityDto>();
                ViewBag.StoneTypes = new List<atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneTypeDto>();
                ViewBag.Stones = new List<atasay_katalog.Models.StoneDto>();
            }
        ...
        // --- Cache Logic START ---
        var cacheKey = CustomerCacheKey("Products");
        ...
        var categoriesResponse = await _mediator.Send(new atasay_katalog.Core.Features.CategoryFeature.Queries.GetAllCategoriesQueryRequest());
        var categoriesList = categoriesResponse.isSuccess ? categoriesResponse.data : new List<atasay_katalog.Models.CategoryDto>();
        ...
        var metalTypeList = ViewBag.MetalTypes as List<atasay_katalog.Core.Features.DefinitionFeature.Queries.MetalTypeDto>;
        if (metalTypeList == null || !metalTypeList.Any())
        {
            ViewBag.MetalTypes = fetchedProducts
                .SelectMany(p => p.ProductMetals ?? new List<atasay_katalog.Core.Features.ProductFeature.Queries.ApiProductMetal>())
                .Where(pm => pm != null && pm.MetalTypeId > 0 && !string.IsNullOrEmpty(pm.MetalTypeName))
                .GroupBy(m => m.MetalTypeId)
                .Select(g => new atasay_katalog.Core.Features.DefinitionFeature.Queries.MetalTypeDto { Id = g.Key, Name = g.First().MetalTypeName })
                .ToList();
        }

        var clarityList = ViewBag.Clarities as List<atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneClarityDto>;
        if (clarityList == null || !clarityList.Any())
        {
            ViewBag.Clarities = fetchedProducts
                .SelectMany(p => p.ProductStones ?? new List<atasay_katalog.Core.Features.ProductFeature.Queries.ApiProductStone>())
                .Where(ps => ps != null && ps.ClarityId.HasValue && !string.IsNullOrEmpty(ps.ClarityName))
                .GroupBy(c => c.ClarityId.GetValueOrDefault())
                .Select(g => new atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneClarityDto { Id = g.Key, Name = g.First().ClarityName })
                .ToList();
        }

        var stoneTypeList = ViewBag.StoneTypes as List<atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneTypeDto>;
        if (stoneTypeList == null || !stoneTypeList.Any())
        {
            // Fallback common stone types since we cannot extract from ApiProductStone
            ViewBag.StoneTypes = new List<atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneTypeDto>
            {
                new atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneTypeDto { Id = 1, Name = "Pırlanta" },
                new atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneTypeDto { Id = 2, Name = "Zümrüt" },
                new atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneTypeDto { Id = 3, Name = "Safir" },
                new atasay_katalog.Core.Features.DefinitionFeature.Queries.StoneTypeDto { Id = 4, Name = "Yakut" }
            };
        }

        try {
            var stoneResponse = await _mediator.Send(new atasay_katalog.Core.Features.ProductFeature.Queries.GetAllStonesQueryRequest());
            ViewBag.Stones = (stoneResponse != null && stoneResponse.isSuccess) ? stoneResponse.data : new List<atasay_katalog.Models.StoneDto>();
        } catch {
            ViewBag.Stones = new List<atasay_katalog.Models.StoneDto>();
        }
